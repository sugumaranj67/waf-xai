...
def run_test(
    ...
    if expect_log:
        assert post == pre + 1, f"{name}: expected log count {pre+1}, got {post}"
        log = get_last_log()
        assert (
            log["attack_type"] == exp_attack_type
        ), f"{name}: attack_type {log['attack_type']} != {exp_attack_type}"
        assert (
            log["severity"] == exp_severity
        ...
        assert post == pre, f"{name}: expected no new log, but count went {pre}→{post}"
    print(f"[PASS] {name}")
